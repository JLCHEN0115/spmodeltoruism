...
## FE twoway
preg1_fe <- spml(formula = formula0, data = tlaged_logged_ct_spdata_long, listw = listw3nn, model = "within", effect = "twoway", lag = TRUE, spatial.error = "none")
...
```{r}
results <- results %>% mutate(tlaged_resid = lag(results[["resid"]], 1))
results <- results %>% mutate(tlagedlaged_resid = lag(results[["resid"]], 2))
results <- results %>% mutate(tlagedlagedlaged_resid = lag(results[["resid"]], 3))
resid_plot1 <-results %>% as.data.frame() %>% ggplot(aes(x=resid, y=tlaged_resid)) + geom_point(size = 1) 
resid_plot2 <- results %>% as.data.frame() %>% ggplot(aes(x=resid, y=tlagedlaged_resid)) + geom_point(size = 1) 
resid_plot3 <-  results %>% as.data.frame() %>% ggplot(aes(x=resid, y=tlagedlagedlaged_resid)) + geom_point(size = 1) 
...
results <- pdata.frame(results, index = c("城市shapefile", "year"))
