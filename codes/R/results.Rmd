...
ct_spdata_long$lagemphotel = splag(x = ct_spdata_long$emphotel, n = 283, t = 9, w = matrix6nn)
```
...
## Create indicators using total arrivals, 20 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_tarl_20 = case_when(
      (tarl - lagtarl)/tarl >= 0.2 ~  1,
      (tarl - lagtarl)/tarl < 0.2 ~ 0
     )
   )

## Create indicators using total arrivals, 30 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_tarl_30 = case_when(
      (tarl - lagtarl)/tarl >= 0.3 ~  1,
      (tarl - lagtarl)/tarl < 0.3 ~ 0
     )
   )

## Create indicators using total arrivals, 40 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_tarl_40 = case_when(
      (tarl - lagtarl)/tarl >= 0.4 ~  1,
      (tarl - lagtarl)/tarl < 0.4 ~ 0
     )
   )
...
## Create indicators using Density, compared to country mean, 20 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_20 = case_when(
      (Density-Country_mean_dens)/Country_mean_dens >= 0.2 ~  1,
      (Density-Country_mean_dens)/Country_mean_dens < 0.2 ~ 0
     )
   )

## Create indicators using Density, compared to country mean, 30 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_30 = case_when(
      (Density-Country_mean_dens)/Country_mean_dens >= 0.3 ~  1,
      (Density-Country_mean_dens)/Country_mean_dens < 0.3 ~ 0
     )
   )

## Create indicators using Density, compared to country mean, 40 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_40 = case_when(
      (Density-Country_mean_dens)/Country_mean_dens >= 0.4 ~  1,
      (Density-Country_mean_dens)/Country_mean_dens < 0.4 ~ 0
     )
   )

## Create indicators using Density, compared to country mean, 50 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_50 = case_when(
      (Density-Country_mean_dens)/Country_mean_dens >= 0.5 ~  1,
      (Density-Country_mean_dens)/Country_mean_dens < 0.5 ~ 0
     )
   )

## Create indicators using Density, compared to country mean, 60 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_60 = case_when(
      (Density-Country_mean_dens)/Country_mean_dens >= 0.6 ~  1,
      (Density-Country_mean_dens)/Country_mean_dens < 0.6 ~ 0
     )
   )

## Create indicators using Density, compared to country mean, 70 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_70 = case_when(
      (Density-Country_mean_dens)/Country_mean_dens >= 0.7 ~  1,
      (Density-Country_mean_dens)/Country_mean_dens < 0.7 ~ 0
     )
   )

## Create indicators using Density, compared to country mean, 80 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_80 = case_when(
      (Density-Country_mean_dens)/Country_mean_dens >= 0.8 ~  1,
      (Density-Country_mean_dens)/Country_mean_dens < 0.8 ~ 0
     )
   )
...
## Crease indicators using hotel numbers, 20 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_hotel_20 = case_when(
      (hotel-laghotel)/laghotel >= 0.2 ~  1,
      (hotel-laghotel)/laghotel < 0.2 ~ 0
     )
   )

## Crease indicators using hotel numbers, 30 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_hotel_30 = case_when(
      (hotel-laghotel)/laghotel >= 0.3 ~  1,
      (hotel-laghotel)/laghotel < 0.3 ~ 0
     )
   )

## Crease indicators using hotel numbers, 40 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_hotel_40 = case_when(
      (hotel-laghotel)/laghotel >= 0.4 ~  1,
      (hotel-laghotel)/laghotel < 0.4 ~ 0
     )
   )

## Crease indicators using hotel numbers, 50 percent
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_hotel_50 = case_when(
      (hotel-laghotel)/laghotel >= 0.5 ~  1,
      (hotel-laghotel)/laghotel < 0.5 ~ 0
     )
   )
...
for (i in 1:9){
  location_test[[i]] <-   identical(ct_spdata_wide$城市shapefile,(as.character(ct_spdata_long$城市shapefile))[(283*(i-1)+1):(283*i)])
}
...
#        as.data.frame() %>%
#        select(!geometry) %>%
#      xlsx::write.xlsx2("/Users/jialiangchen/Documents/spmodeltoruism/codes/tworegime/ct_spdata_long.xlsx")
...
tlaged_logged_ct_spdata_long <- tlaged_ct_spdata_long %>%
  mutate(across(c(tarl:inrev, hprice:road, invest:lagpop, laginvest:lagavexp, lagemphotel), \(x) log(x+1)))
...
1. 
```{r}
formula <- trev ~ emphotel + invest + hotel + spot5A + lagemphotel + laginvest + laghotel + lagspot5A
```

3. **nosplag**:
```{r}
formula_nosplag <- trev ~ emphotel + invest + hotel + spot5A + tlagtrev
```
...
```{r}
preg_nonspatial <- plm(formula_nosplag, data = tlaged_logged_ct_spdata_long, index = c("indicator", "year"), model = "pooling")
...
```{r}
preg0 <- spml(formula = formula, data = tlaged_logged_ct_spdata_long, index = c("indicator", "year"), listw = listw6nn, model = "pooling", lag = TRUE, spatial.error = "none")
...
```{r}
## FE individual
preg1_fe_ind <- spml(formula = formula, data = tlaged_logged_ct_spdata_long, listw = listw6nn, model = "within", effect = "individual", lag = TRUE, spatial.error = "none", index = c("indicator", "year"))

## FE time
preg1_fe_time <- spml(formula = formula, data = tlaged_logged_ct_spdata_long, listw = listw6nn, model = "within", effect = "time", lag = TRUE, spatial.error = "none", index = c("indicator", "year"))

## FE twoway
preg1_fe_twoway <- spml(formula = formula, data = tlaged_logged_ct_spdata_long, listw = listw6nn, model = "within", effect = "twoway", lag = TRUE, spatial.error = "none", index = c("indicator", "year"))

## see what if we estimate the random effect model
preg1_re <- spml(formula = formula, data = tlaged_logged_ct_spdata_long, index = c("indicator", "year"), listw = listw6nn, model = "random", lag = TRUE, spatial.error = "none")

summary(preg1_fe_ind)
summary(preg1_fe_time)
summary(preg1_fe_twoway)
summary(preg1_re)
```
...
```{r}
bsktest(formula, data = tlaged_logged_ct_spdata_long, listw = listw6nn, index = c("indicator", "year"), test = "CLMlambda")
```
$H_{0}:$ random effect
```{r}
bsktest(formula, data = tlaged_logged_ct_spdata_long, listw = listw6nn, index = c("indicator", "year"), test = "CLMmu")
```
...
sphtest(x = formula, data = tlaged_logged_ct_spdata_long, listw = listw6nn, index = c("indicator", "year"), spatial.model = "lag", method = "ML")
```

1. These tests are panel versions of the LM test for spatial dependence of Anselin (1996).
```{r}
slmtest(formula = formula, data = tlaged_logged_ct_spdata_long, listw = listw6nn, index = c("indicator", "year"), model = "within", test = "lme")
```
```{r}
slmtest(formula = formula, data = tlaged_logged_ct_spdata_long, listw = listw6nn, index = c("indicator", "year"), model = "within", test = "lml")
```
...
sum(results$y - tlaged_logged_ct_spdata_long$trev) # should be basically 0, up to some rounding error
...
### Hausman test
```{r}
sphtest(preg1_fe_twoway, preg1_re)
