...
```{r}
tlaged_logged_ct_spdata_long <- panel_ct_spdata_long
...
preg0_nosplag <- spml(formula = formula_nosplag, data = tlaged_logged_ct_spdata_long, index = c("城市shapefile", "year"), listw = listw3nn, model = "pooling", lag = FALSE, spatial.error = "none")
...
```{r}
## FE individual
preg1_fe_ind <- spml(formula = formula0, data = tlaged_logged_ct_spdata_long, listw = listw3nn, model = "within", effect = "individual", lag = TRUE, spatial.error = "none", index = c("城市shapefile", "year"))

## FE time
preg1_fe_time <- spml(formula = formula0, data = tlaged_logged_ct_spdata_long, listw = listw3nn, model = "within", effect = "time", lag = TRUE, spatial.error = "none", index = c("城市shapefile", "year"))

## FE twoway
preg1_fe <- spml(formula = formula0, data = tlaged_logged_ct_spdata_long, listw = listw3nn, model = "within", effect = "twoway", lag = TRUE, spatial.error = "none", index = c("城市shapefile", "year"))
...
preg1_re <- spml(formula = formula0, data = tlaged_logged_ct_spdata_long, index = c("城市shapefile", "year"), listw = listw3nn, model = "random", lag = TRUE, spatial.error = "none")
...
results <- read.xlsx("/Users/jialiangchen/Documents/spmodeltoruism/codes/tworegime/results.xlsx", 1) %>% data.table::transpose(make.names = "OriginalVariableNames") %>% mutate(城市shapefile = stacked_tlaged_logged_ct_spdata_long$城市shapefile, year = stacked_tlaged_logged_ct_spdata_long$year) %>% select(c("城市shapefile", "year", "y", "yhat", "resid")) 
...
ct_spdata_long_result_auged <- left_join(ct_spdata_long, results, by = c("城市shapefile", "year"))
...
```{r}
panel_results <- pdata.frame(results, index = c("城市shapefile", "year"))
...
```{r}
slmtest(resid ~ 1, data = ct_spdata_long_result_auged, listw = listw3nn, index = c("城市shapefile", "year"), model="pooling", test = "lml")
```
