...
pdata.frame(ct_spdata_long, index = c("城市shapefile", "year"))
## summarize the data
...
lagGDPpc <- slag(pdata.frame(ct_spdata_long, index = c("城市shapefile", "year"))$GDPpc, listw = listw4nn) %>% as.numeric( )

ct_spdata_long$lagGDPpc <- lagGDPpc

lagslry <- slag(pdata.frame(ct_spdata_long, index = c("城市shapefile", "year"))$slry, listw = listw4nn) %>% as.numeric( )

ct_spdata_long$lagslry <- lagslry

lagtarl <- slag(pdata.frame(ct_spdata_long, index = c("城市shapefile", "year"))$tarl, listw = listw4nn) %>% as.numeric( )
...
```{r}
pslry <- pdata.frame(ct_spdata_long, c("城市shapefile", "year"))$slry

formula4 <- log(1+tarl) ~ log(1 + GDPpc) + log(1 + slag(pslry, listw = listw4nn)) + log(1+invest) + log(1+hotel) + log(1+spot5A)+ log(1+scenum) + log(1+pop) + log(1+bus)
```
...
```{r}
preg1_fe <- spml(formula = formula4, data = ct_spdata_long, index = c("城市shapefile", "year"), listw = listw4nn, model = "within", effect = "individual", lag = TRUE, spatial.error = "none")
preg1_re <- spml(formula = formula4, data = ct_spdata_long, index = c("城市shapefile", "year"), listw = listw4nn, model = "random",  effect = "individual", lag = TRUE, spatial.error = "none")
...
# ```{r}
# preg2_fe <- spgm(formula = formula4, data = ct_spdata_long, index = c("城市shapefile", "year"), listw = listw4nn, lag = TRUE, spatial.error = FALSE,  model = "within",  verbose = TRUE,  endog = ~ log(1+tavexp), instruments = ~log(1+hprice) + log(1+emphotel) + log(1+taxi))
# ##, endog = ~ log(1+tavexp), instruments = ~log(1+hprice))
# 
