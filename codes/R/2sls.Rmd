...
## panel
## CAUTION: panel structure will mess things up when calculateing Moran's I
...
```{r}
## Create spatial lag for some variable
lagGDPpc <- slag(panel_ct_spdata_long$GDPpc, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$lagGDPpc <- lagGDPpc
panel_ct_spdata_long$lagGDPpc <- lagGDPpc

lagslry <- slag(panel_ct_spdata_long$slry, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$lagslry <- lagslry
panel_ct_spdata_long$lagslry <- lagslry

lagtarl <- slag(panel_ct_spdata_long$tarl, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$lagtarl <- lagtarl
panel_ct_spdata_long$lagtarl <- lagtarl

lagpop <- slag(panel_ct_spdata_long$pop, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$lagpop <- lagpop
panel_ct_spdata_long$lagpop <- lagpop

lagterti <- slag(panel_ct_spdata_long$terti, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$lagterti <- lagterti
panel_ct_spdata_long$lagterti <- lagterti

laginvest <- slag(panel_ct_spdata_long$invest, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$laginvest <- laginvest
panel_ct_spdata_long$laginvest <- laginvest

lagtaxi <- slag(panel_ct_spdata_long$taxi, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$lagtaxi <- lagtaxi
panel_ct_spdata_long$lagtaxi <- lagtaxi

laghotel <- slag(panel_ct_spdata_long$hotel, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$laghotel <- laghotel
panel_ct_spdata_long$laghotel <- laghotel

lagspot5A <- slag(panel_ct_spdata_long$spot5A, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$lagspot5A <- lagspot5A
panel_ct_spdata_long$lagspot5A <- lagspot5A

laggrnld <- slag(panel_ct_spdata_long$grnld, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$laggrnld <- laggrnld
panel_ct_spdata_long$laggrnld <- laggrnld

lagtavexp <- slag(panel_ct_spdata_long$tavexp, listw = listw3nn) %>% as.numeric( )
ct_spdata_long$lagtavexp <- lagtavexp
panel_ct_spdata_long$lagtavexp <- lagtavexp
```

lag them again.
```{r}
## Create spatial lag for some variable
lag2GDPpc <- slag(panel_ct_spdata_long$lagGDPpc, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2GDPpc <- lag2GDPpc
panel_ct_spdata_long$lag2GDPpc <- lag2GDPpc

lag2slry <- slag(panel_ct_spdata_long$lagslry, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2slry <- lag2slry
panel_ct_spdata_long$lag2slry <- lag2slry

lag2tarl <- slag(panel_ct_spdata_long$lagtarl, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2tarl <- lag2tarl
panel_ct_spdata_long$lag2tarl <- lag2tarl

lag2pop <- slag(panel_ct_spdata_long$lagpop, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2pop <- lag2pop
panel_ct_spdata_long$lag2pop <- lag2pop

lag2terti <- slag(panel_ct_spdata_long$lagterti, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2terti <- lag2terti
panel_ct_spdata_long$lag2terti <- lag2terti

lag2invest <- slag(panel_ct_spdata_long$laginvest, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2invest <- lag2invest
panel_ct_spdata_long$lag2invest <- lag2invest

lag2taxi <- slag(panel_ct_spdata_long$lagtaxi, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2taxi <- lag2taxi
panel_ct_spdata_long$lag2taxi <- lag2taxi

lag2hotel <- slag(panel_ct_spdata_long$laghotel, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2hotel <- lag2hotel
panel_ct_spdata_long$lag2hotel <- lag2hotel

lag2spot5A <- slag(panel_ct_spdata_long$lagspot5A, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2spot5A <- lag2spot5A
panel_ct_spdata_long$lag2spot5A <- lag2spot5A

lag2grnld <- slag(panel_ct_spdata_long$laggrnld, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2grnld <- lag2grnld
panel_ct_spdata_long$lag2grnld <- lag2grnld

lag2tavexp <- slag(panel_ct_spdata_long$lagtavexp, listw = listwnb) %>% as.numeric( )
ct_spdata_long$lag2tavexp <- lag2tavexp
panel_ct_spdata_long$lag2tavexp <- lag2tavexp
```
...
for (i in 7:ncol(panel_tlaged_logged_ct_spdata_long)) {
...
tlaged_logged_ct_spdata_long <- lapply(tlaged_logged_ct_spdata_long, function(x){attr(x, "index") <- NULL; x}) %>% as.data.frame()

summary(tlaged_logged_ct_spdata_long)

## Drop all observations with missing values 
panel_tlaged_logged_ct_spdata_long <- na.omit(panel_tlaged_logged_ct_spdata_long)
...
tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>%
  mutate(across(c(tarl:inrev, hprice:road, invest:lagpop, laginvest:lag2tavexp), \(x) log(x+1)))
...
```{r}
tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(Dtral5gdp = lagtarl * indicator_gdpps5)

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(neDtral5gdp = lagtarl * (1 - indicator_gdpps5))

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(Dtral10gdp = lagtarl * indicator_gdpps10)

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(neDtral10gdp = lagtarl * (1-indicator_gdpps10))

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(Dtralgdp = lagtarl * indicator_gdp)

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(neDtralgdp = lagtarl * (1 - indicator_gdp))

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(Dtralgdpne5to0 = lagtarl * indicator_gdpne5to0)

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(neDtralgdpne5to0 = lagtarl * (1 - indicator_gdpne5to0))

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(Dtral_tarl = lagtarl * indicator_tarl)

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(neDtral_tarl = lagtarl * (1 - indicator_tarl))

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(Dtral_5a = lagtarl * indicator_5A)

tlaged_logged_ct_spdata_long <- tlaged_logged_ct_spdata_long %>% mutate(neDtral_5a = lagtarl * (1 - indicator_5A))
```
...
```{r}
test <- tarl ~ Dtral5gdp + neDtral5gdp + GDPpc + slry + pop + terti + invest + taxi + hotel + spot5A + grnld + tavexp + lagGDPpc + lagslry + lagpop + lagterti + laginvest + lagtaxi + laghotel + lagspot5A + laggrnld + lagtavexp |. - Dtral5gdp - neDtral5gdp + GDPpc + slry + pop + terti + invest + taxi + hotel + spot5A + grnld + tavexp + lagGDPpc + lagslry + lagpop + lagterti + laginvest + lagtaxi + laghotel + lagspot5A + laggrnld + lagtavexp+ lag2GDPpc + lag2slry + lag2pop + lag2terti + lag2invest + lag2taxi + lag2hotel + lag2spot5A + lag2grnld + lag2tavexp


plm(formula = test, index= c("城市shapefile", "year"), data = tlaged_logged_ct_spdata_long, model = "within", effect = "twoway") %>% summary()
```
...
```{r}
first_stage <- plm(formula = neDtral5gdp ~ lag2GDPpc + lag2slry + lag2pop + lag2terti + 
    lag2invest + lag2taxi + lag2hotel + lag2spot5A + lag2grnld + 
    lag2tavexp, data = tlaged_logged_ct_spdata_long, effect = "twoway", 
...
```{r}
plm(formula = tarl ~ Dtral5gdp + neDtral5gdp + GDPpc + slry + pop + terti + invest + taxi + hotel + spot5A + grnld + tavexp + lagGDPpc + lagslry + lagpop + lagterti + laginvest + lagtaxi + laghotel + lagspot5A + laggrnld + lagtavexp + GDPpc + slry + pop + terti + invest + taxi + hotel + spot5A + grnld + tavexp + lagGDPpc + lagslry + lagpop + lagterti + laginvest + lagtaxi + laghotel + lagspot5A + laggrnld + lagtavexp, data = tlaged_logged_ct_spdata_long, effect = "twoway", 
