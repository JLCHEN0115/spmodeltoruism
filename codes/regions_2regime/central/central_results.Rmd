...
ct_spdata_long$Region_mean_dens <- numeric(720)

for (i in 1:9){
  ct_spdata_long$Region_mean_dens[(80*(i-1)+1):(80*i)] <- 
...
ct_spdata_long$lagdens = splag(x = ct_spdata_long$Density, n = 80, t = 9, w = matrix6nn)
ct_spdata_long$lagemphotel = splag(x = ct_spdata_long$emphotel, n = 80, t = 9, w = matrix6nn)
```
...
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_20 = case_when(
      (Density-Region_mean_dens)/Region_mean_dens >= 0.2 ~  1,
      (Density-Region_mean_dens)/Region_mean_dens < 0.2 ~ 0
     )
   )
...
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_30 = case_when(
      (Density-Region_mean_dens)/Region_mean_dens >= 0.3 ~  1,
      (Density-Region_mean_dens)/Region_mean_dens < 0.3 ~ 0
     )
   )
...
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_40 = case_when(
      (Density-Region_mean_dens)/Region_mean_dens >= 0.4 ~  1,
      (Density-Region_mean_dens)/Region_mean_dens < 0.4 ~ 0
     )
   )
...
ct_spdata_long <- ct_spdata_long %>%
   mutate(
     indicator_dens_mean_50 = case_when(
      (Density-Region_mean_dens)/Region_mean_dens >= 0.5 ~  1,
      (Density-Region_mean_dens)/Region_mean_dens < 0.5 ~ 0
     )
   )
...
for (i in 1:9){
  location_test[[i]] <-   identical(ct_spdata_wide$城市shapefile,(as.character(ct_spdata_long$城市shapefile))[(80*(i-1)+1):(80*i)])
}
...
#        as.data.frame() %>%
#        select(!geometry) %>%
#      xlsx::write.xlsx2("/Users/jialiangchen/Documents/spmodeltoruism/codes/regions_2regime/central/central_spdata_long.xlsx")
#  
#  ## output weight matrix (4nn)
#  matrix4nn %>% as.data.frame() %>% write.xlsx2("/Users/jialiangchen/Documents/spmodeltoruism/codes/regions_2regime/central/4nnmatrix.xlsx")
```
...
```{r}
preg_nonspatial <- plm(formula = tarl ~ GDPpc + slry + pop + terti + invest + taxi + hotel + spot5A + grnld + tavexp, data = tlaged_logged_ct_spdata_long, index = c("城市shapefile", "year"), model = "pooling")
...
```{r}
preg0 <- spml(formula = formula0, data = tlaged_logged_ct_spdata_long, index = c("indicator", "year"), listw = listw3nn, model = "pooling", lag = TRUE, spatial.error = "none")
...
```{r}
## FE individual
preg1_fe_ind <- spml(formula = formula0, data = tlaged_logged_ct_spdata_long, listw = listw3nn, model = "within", effect = "individual", lag = TRUE, spatial.error = "none", index = c("indicator", "year"))

## FE time
preg1_fe_time <- spml(formula = formula0, data = tlaged_logged_ct_spdata_long, listw = listw3nn, model = "within", effect = "time", lag = TRUE, spatial.error = "none", index = c("indicator", "year"))

## FE twoway
preg1_fe <- spml(formula = formula0, data = tlaged_logged_ct_spdata_long, listw = listw3nn, model = "within", effect = "twoway", lag = TRUE, spatial.error = "none", index = c("indicator", "year"))

## see what if we estimate the random effect model
preg1_re <- spml(formula = formula0, data = tlaged_logged_ct_spdata_long, index = c("indicator", "year"), listw = listw3nn, model = "random", lag = TRUE, spatial.error = "none")
...
sphtest(x = formula_nosplag, data = tlaged_logged_ct_spdata_long, listw = listw3nn, index = c("城市shapefile", "year"), spatial.model = "lag", method = "ML")
```

1. These tests are panel versions of the LM test for spatial dependence of Anselin (1996).
```{r}
slmtest(formula = formula0, data = tlaged_logged_ct_spdata_long, listw = listw3nn, index = c("城市shapefile", "year"), model = "within", test = "lme")
```
```{r}
slmtest(formula = formula0, data = tlaged_logged_ct_spdata_long, listw = listw3nn, index = c("城市shapefile", "year"), model = "within", test = "lml")
```
...
sum(results$y - stacked_tlaged_logged_ct_spdata_long$tarl) # should be basically 0, up to some rounding error
str(results)
...
for (i in 1:283){
  ser_corr[[i]] <- (results %>% filter(城市shapefile == cities_names[[i]]) %>% select(resid) %>% unlist() %>% EnvStats::serialCorrelationTest(test="AR1.yw") )$p.value
}

mean(ser_corr < 0.1)
```

### lm test for spatial correlation in the residual

```{r}
slmtest(resid ~ 1, data = ct_spdata_long_result_auged, listw = listw3nn, index = c("indicator", "year"), model="pooling", test = "lml")
```
...
## visual test 
results <- results %>% mutate(srqt_standized_resid = sqrt(standized_resid))
...
```{r}
## Calculate likelihood ratio test statistics for individaul fe
lr_stat_slag_indep <- 2 * (preg0$logLik - preg0_nosplag$logLik)
lr_stat_slag_indep
