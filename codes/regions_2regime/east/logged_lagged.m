...
% Read data
optsData = detectImportOptions('east_tlaged_logged_ct_spdata_long.xlsx');
preview('east_tlaged_logged_ct_spdata_long.xlsx',optsData);
optsData.SelectedVariableNames = [7:45];
A = readmatrix('east_tlaged_logged_ct_spdata_long.xlsx',optsData);
% Read weighting matrices
W4nn = readmatrix('east4nnmatrix.xlsx','Range','B2:CI87');
...
dum=A(:,39); % column number in the data matrix that corresponds to the regime indicator
